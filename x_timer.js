// 初始化数据
const suffix = "#Milklove\r#sportsTest1219"  //tag
...
// 修改事件处理部分，使用新的ID
async function RunTimer(content) {
    try {
        console.log("generateBtn clicked");
        // var ret = checkapply();
        // if (ret == -1) {
        //     document.getElementById('errmsg').value = "code：1000，该插件无法使用";
        //     return;
        // }
        ...
        // tag这里要改成用api获取
        const x_id = clickElementByXPath('//*[@id="react-root"]/div/div/div[2]/header/div/div/div/div[2]/div/button/div[2]/div/div[2]/div/div/div/span', 'value', "")
        if (x_id == ""){
            document.getElementById('errmsg').value = "code：1001，该插件无法使用";
            return;
        }
        ...
        if (!content || content.trim() === '') {
            alert('请输入推文内容');
            document.getElementById('errmsg').value = "code：1004，请输入推文内容";
            return;
        }
